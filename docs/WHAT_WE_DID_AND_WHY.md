...
## 15. Phase 5 benchmark: the honest number is refusal, not multi-hop

**What:** `scripts/benchmark.py` + a 53-question labeled set (`hops` strata + a `gold` answer key). Runs the hybrid answerer and a vector-only baseline over the same set; reports answer accuracy by hop count plus latency and modelled cost. Hybrid **0.83** vs. vector-only **0.68** overall.

**Design decisions and why:**
- **Fact-checklist grading, not an LLM judge.** The calibrated judge is BASWE Project 4, which isn't built. A deterministic checklist (each question = 1–3 required facts, substring match after light normalisation, and an in-scope answer that hedges "does not contain…" can't score `correct`) is reproducible and honest about being strict on phrasing — every `wrong`/`partial` was read by hand. An uncalibrated judge would just move "is this number trustworthy?" one level down.
- **Baseline = the same answerer forced to the vector path.** `answer_question(force_path="vector")` skips the router and the graph; evidence-building, synthesis and citation validation are byte-identical. So the accuracy delta is attributable to exactly one thing: graph vs. no graph.
- **Cost is modelled, not real.** Free tier = $0 actual. The benchmark tallies `usage_metadata` token counts and multiplies by Google's published paid Flash-Lite price ($0.10 / $0.40 per 1M in/out). Latency is real model + retrieval time with the free-tier pacing sleeps subtracted.
- **Per-call pacing + 429 retry.** flash-lite free tier is 15 rpm and a question is ~2–3 Gemini calls (×2 systems); the wrapper spaces every call ~4.5 s and backs off on 429 rather than pacing per question. The first two full runs each took ~16 min of model time inside a ~1-hour wall clock.

**The honest result.** The spec expects "a widening gap as hops increase." We got near-parity on every in-scope stratum — single-hop 0.89 vs 0.94, two-hop 0.92 vs 0.83, three-hop 0.67 vs 0.67, aggregation 0.56 vs 0.56 — all within one question at these sample sizes. **The entire +0.15 overall gain is out-of-scope refusal: 8/8 for the hybrid, 0/8 for the vector-only baseline**, which has no mechanism to decline and answers every unanswerable question confidently. Why the graph underperforms: one-company corpus ⇒ thin graph (only ~4 of the original 27 questions truly need entity traversal; `SUPPLIES` had zero instances, §4). The real accuracy ceiling is **retrieval recall** (≈0.5, §10) — most remaining misses on *both* systems are the retriever not surfacing the chunk (DMA compliance date, contractual-obligations table, risk-factor category headings). The graph even hurt once: q019, an App Store legal question the router sent to `graph`-only, returned commission-structure facts and missed the Epic Games narrative the vector path answered correctly. This is the "what didn't work" the spec asks for, and it's in the README.

---

## Summary framing

Phases 1 and 2 have working, idempotent, cost-conscious pipelines, and the three items originally skipped when they were first marked "complete" — ontology constraint (§4), entity resolution (§5), and retrieval-quality measurement (§10) — are now all done and verified. The vector index was also moved from `ivfflat` to HNSW (§8, `ef_search` then swept and confirmed flat) and the two stores are now linked by entity (§9). Phase 3 (§13) is built, tested against real output rather than assumed, and closes the remaining gaps against the original spec: relationship-type count, ingestion-time resolution, confidence fallback, routing log, extraction retry breadth (§3), few-shot router prompt and a query-type-keyed template library (§13). Phase 4 (§14) merges both retrieval paths into one grounded answer behind a FastAPI `POST /ask`, with every citation validated to resolve to a retrieved chunk. Phase 5 (§15) benchmarks the hybrid answerer against a vector-only baseline: 0.83 vs. 0.68 over 53 questions, with the honest finding that the gain is refusal handling, not multi-hop accuracy, on a corpus whose graph is too thin to move the needle. **All five phases are complete.**

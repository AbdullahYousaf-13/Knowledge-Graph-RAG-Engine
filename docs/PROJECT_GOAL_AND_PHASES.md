...
- Labeled question set with hop-count strata. — **Done.** `data/eval/retrieval_queries.jsonl` = 53 questions with `hops` (1/2/3/agg/0) and a `gold` answer key (fact checklist or `must_refuse`). The 18 multi-hop questions are entity-connection questions (connection chains, comparisons across entities, relationship tallies) that route to graph/both.
- Compare the hybrid system to a vector-only baseline. — **Done.** `answer.answer_question()` vs. `answer.answer_question(force_path="vector")` (skips router + graph; identical synthesis + citation validation). Deterministic fact-checklist grading — no LLM judge (the calibrated judge is a separate project).
- Report accuracy by hop count, plus latency and cost per query. — **Done.** Hybrid **0.85** vs. vector-only **0.68** overall (+0.17). By stratum: single-hop 0.89 vs 0.94, two-hop 0.75 vs 0.67, three-hop 1.00 vs 1.00, aggregation 0.67 vs 0.56, out-of-scope 1.00 vs 0.00. Latency: hybrid p50 8.5 s / p95 19.8 s vs. vector-only p50 6.4 s / p95 9.4 s. Modelled cost: $1.25 vs $0.49 per 1k queries ($0 actual on the free tier); hybrid uses ~2.8× the tokens. One-time ingestion: ~225 extraction calls, $0. `data/eval/results/benchmark_20260901T114932Z.json`.
- Put the benchmark table at the top of the README. — **Done.**

**Phase 5 is complete.** Honest read: the hybrid's edge is concentrated where a graph is for — **out-of-scope refusal (+1.00; a plain vector RAG can't decline, 0/8), aggregation over relationships (+0.11), and a real but noisy multi-hop gain (+0.08 two-hop)**. Single-hop is parity. It costs ~2.8× the tokens and runs ~40% slower. Caveats: run-to-run variance is real at n = 6–12 per stratum; the graph having a fact doesn't guarantee the answer uses it (q025); retrieval recall (≈0.5) is the ceiling for both systems. See the README "what didn't work" and `WHAT_WE_DID_AND_WHY.md` §15.
